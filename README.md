...
| `浮生记 留守` | 原地过一天 |
| `浮生记 买/卖 <货\|序号> <数\|全>` | 交易。货品可用行情表序号（1~8）、简称、别名，如「卖 7 全」 |
...
6. **行情对照评价**：每条暴涨暴跌事件按力度折算成「景气点」，一局攒下来就是这局的行情档位（冷清/平淡/红火/疯狂）。结算特刊按「你赶上的行情」评价你的战绩——冷清局抠出小钱是本事，疯狂局只捡个零头会被点破；欠债离场与身故是垫底档。评级线用 14,400 局机器人模拟校准，同样的钱在不同的局里说法不同。住院天数不计入个人行情（看得见吃不着，不算你的机遇）。
7. **AI 说书人**（可选，默认关）：结算后由 LLM 生成京味收场白，单人局与多人局各一套提示词，数值规则与上述行情评级一并交给模型，夸损有据。可在配置里指定供应商，失败自动累计重试 3 次，再失败回退到会话默认模型，仍失败会在群里明确提示「AI 总结失败」，绝不静默吞掉。
...
- 原版「我要逛京城/我要进地铁」按钮是纯视觉彩蛋：把十个地名在地铁站名与地面街道名之间互换，行情、事件全不变。文字指令版没有按钮，未保留。
...
tests/   180 项测试：规则逐条验证、序列化回环、多种子随机模糊、插件层集成
```
